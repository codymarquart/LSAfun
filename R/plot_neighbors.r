...
plot_neighbors <- function(x,n,connect.lines=0,
  ...
  if(class(tvectors) == "matrix"){
    if(class(x) == "character"){
      ...
      near <- names(neighbors(satz1vec,n, tvectors=tvectors, breakdown=breakdown))
    }
      
    if(class(x) == "numeric") {
      satz1vec  <- x
      near      <- names(neighbors(x,n,tvectors=tvectors, breakdown=breakdown))
    }

    nearwords <- paste(near,collapse=" ")
    cos.near  <- multicos(nearwords,tvectors=tvectors, breakdown=FALSE)
